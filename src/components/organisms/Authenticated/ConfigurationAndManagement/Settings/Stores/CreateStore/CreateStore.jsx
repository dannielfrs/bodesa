...
export default function CreateStore ({ disabled, title }) {
  const methods = useForm()
  const router = useRouter()

  const onSubmit = () => {
    Loading('Guardando tienda')
    setTimeout(() => {
      Success('Tienda guardada', 'La tienda se guardo de forma correcta')
    }, 2000)
    setTimeout(() => {
      Alert('Ocurrio un error', 'La tienda no se guardo')
    }, 3000)
    setTimeout(() => {
      Swal.close()
      router.push('/system/configuration-and-management/settings/stores')
    }, 4000)
  }
  const inputCode = {
    name: 'code',
              ...
              <div className={styles.Buttons}>
                <div style={{ width: '100px' }}>
                  <Button
                    ...
                    type='button'
                    label='Regresar'
                    onClick={() => router.push('/system/configuration-and-management/settings/stores')}
                  />
                </div>
                {!disabled &&
                  <div style={{ width: '100px' }}>
