...
import styles from './EstablishmentsTypes.module.scss'
...
export default function EstablishmentsTypes () {
  ...
  const { data, loading, setData } = usePostDataRefresh('ocr/configurations/catalogs/establishment-types/data-table', selectCompany, refresh)
  ...
  const handleActive = (rowData) => {
    changeStatusService(`ocr/configurations/catalogs/establishment-types/${rowData.id}/change-status`, setRefresh, rowData.status)
  }

  const OnDelete = (rowData, setData) => {
    DeleteRow(rowData.id, `ocr/configurations/catalogs/establishment-types/${rowData.id}`, rowData.name, setData)
  }
  ...
  const actionsBodyTemplate = (rowData) => {
    return (
      <ActionsBody
        ...
        urlShow={`/system/ocr/settings/catalogs/establishments-types/${rowData.id}`}
        urlEdit={`/system/ocr/settings/catalogs/establishments-types/${rowData.id}/edit`}
  ...
  const dataTableColumns = [
    ...
    { id: 2, field: 'status', header: 'Estatus', body: statusColumnTemplate, sortable: true, w: '180px', h: '51px' },
  ...
  return (
    <FormHookProvider methods={methods} method='POST' onSubmit={onSubmit}>
      <div className={styles.Templates}>
        <HeaderTable title='Tipos establecimientos' setSearch={setSearch} dropCompanies={false} handleNew={() => router.push('/system/ocr/settings/catalogs/establishments-types/create')} />
